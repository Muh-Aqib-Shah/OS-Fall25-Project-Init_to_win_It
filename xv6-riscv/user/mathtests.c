...
int test_maths() {
    ...
    // Helper to print function header
    #define START_FUNC_TEST(name) \
        ok = 0; fail = 0; \
        printf("\n--- Testing %s ---\n", name);

    // Helper to print function summary
    #define END_FUNC_TEST(name) \
    ...
    // ---------------- FABS ----------------
    START_FUNC_TEST("fabs")
    chk("fabs(-3.5)", xv6m_fabsf(-3.5f), 3.5f, T, &ok, &fail);
    END_FUNC_TEST("fabs")
    

    // ---------------- SQRT ----------------
    START_FUNC_TEST("sqrt")
    chk("sqrt(0)", xv6m_sqrtf(0.f), 0.f, T, &ok, &fail);
    chk("sqrt(9)", xv6m_sqrtf(9.f), 3.f, T, &ok, &fail);
#if HAS_GEN_DATA
    for (int i = 0; i < SQRT_N; i++)
        chk("sqrt_gen", xv6m_sqrtf(sqrt_in[i]), sqrt_out[i], 1e-5f, &ok, &fail);
#endif
    END_FUNC_TEST("sqrt")
    ...
    for (int i = 0; i < EXP_N; i++)
        chk("exp_gen", xv6m_expf(exp_in[i]), exp_out[i], 1e-5f, &ok, &fail);
#endif
    END_FUNC_TEST("exp")
    ...
    for (int i = 0; i < POW_N; i++)
        chk("pow_gen", xv6m_powf(pow_x[i], pow_y[i]), pow_out[i], 1e-4f, &ok, &fail);
#endif
    END_FUNC_TEST("pow")
    ...
    for (int i = 0; i < SIN_N; i++)
        chk("sin_gen", xv6m_sinf(sin_in[i]), sin_out[i], 1e-5f, &ok, &fail);
#endif
    END_FUNC_TEST("sin")
    ...
    for (int i = 0; i < COS_N; i++)
        chk("cos_gen", xv6m_cosf(cos_in[i]), cos_out[i], 1e-5f, &ok, &fail);
#endif
    END_FUNC_TEST("cos")

    // ---------------- FABS_GEN ----------------
#if HAS_GEN_DATA
    START_FUNC_TEST("fabs_gen")
    for (int i = 0; i < FABS_N; i++)
        chk("fabs_gen", xv6m_fabsf(fabs_in[i]), fabs_out[i], 0.0f, &ok, &fail);
    END_FUNC_TEST("fabs_gen")
#endif

    // ---------------- TOTAL SUMMARY ----------------
    printf("\n=== TOTAL MATH TESTS: %d passed, %d failed ===\n", total_ok, total_fail);
