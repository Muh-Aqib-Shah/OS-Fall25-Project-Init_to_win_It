...
// At the top of your file, after #define NUM_THREADS
#define NUM_THREADS 1  // Increased from 2

// Global static storage to avoid malloc/free overhead
...
//     thread_exit();
// }
// Improved qkv_parallel - no malloc/free
...
//     if (n_heads < NUM_THREADS || dim < 256) {
...
//         g_qkv_args[t].wk = wk;
//         g_qkv_args[t].wv = wv;
//         g_qkv_args[t].dim = dim;
//         g_qkv_args[t].head_start = current_head;
//         g_qkv_args[t].head_end = current_head + heads_for_this_thread;
//         g_qkv_args[t].head_size = head_size;

//         g_qkv_thread_ids[t] = thread_create(qkv_worker, &g_qkv_args[t]);
//         current_head += heads_for_this_thread;
//     }

//     for (int t = 0; t < NUM_THREADS; t++) {
//         if (g_qkv_thread_ids[t] >= 0) {
//             thread_join(g_qkv_thread_ids[t]);
//         }
//     }
// }
...
static int g_matmul_thread_ids[NUM_THREADS];
static MatmulArgs g_matmul_args[NUM_THREADS];
//static int g_qkv_thread_ids[NUM_THREADS];
//static QKVArgs g_qkv_args[NUM_THREADS];
static int g_attention_thread_ids[NUM_THREADS];
static AttentionArgs g_attention_args[NUM_THREADS];


// Improved matmul_parallel - no malloc/free, higher threshold
void matmul_parallel(float* output, float* input, float* weight, int n, int d) {
    // Only use threading for large matrices (increased threshold)
    if (d < 512) {
        for (int i = 0; i < d; i++) {
            float val = 0.0f;
            for (int j = 0; j < n; j++) {
    ...
    // Use static storage instead of malloc
    ...
    for (int t = 0; t < NUM_THREADS; t++) {
        ...
        g_matmul_args[t].output = output;
        g_matmul_args[t].input = input;
        g_matmul_args[t].weight = weight;
        g_matmul_args[t].n = n;
        g_matmul_args[t].d = d;
        g_matmul_args[t].row_start = current_row;
        g_matmul_args[t].row_end = current_row + rows_for_this_thread;

        g_matmul_thread_ids[t] = thread_create(matmul_worker, &g_matmul_args[t]);
        
        if (g_matmul_thread_ids[t] < 0) {
            // Thread creation failed - do remaining work sequentially
            for (int j = 0; j < created_count; j++) {
                thread_join(g_matmul_thread_ids[j]);
            }
    ...
    // Join all threads
    for (int t = 0; t < NUM_THREADS; t++) {
        thread_join(g_matmul_thread_ids[t]);
    }
}
...
// Improved attention_parallel - no malloc/free
void attention_parallel(RunState* s, int n_heads, int head_size,
    ...
    for (int t = 0; t < NUM_THREADS; t++) {
        int heads_for_this_thread = heads_per_thread + (t < extra_heads ? 1 : 0);
        g_attention_args[t].att = s->att;
        g_attention_args[t].q = s->q;
        g_attention_args[t].key_cache = s->key_cache;
        g_attention_args[t].xb = s->xb;
        g_attention_args[t].value_cache = s->value_cache;
        g_attention_args[t].seq_len = seq_len;
        g_attention_args[t].head_size = head_size;
        g_attention_args[t].pos = pos;
        g_attention_args[t].loff = loff;
        g_attention_args[t].dim = dim;
        g_attention_args[t].head_start = current_head;
        g_attention_args[t].head_end = current_head + heads_for_this_thread;

        g_attention_thread_ids[t] = thread_create(attention_worker, &g_attention_args[t]);
        current_head += heads_for_this_thread;
    }

    for (int t = 0; t < NUM_THREADS; t++) {
        if (g_attention_thread_ids[t] >= 0) {
            thread_join(g_attention_thread_ids[t]);
        }
    }
}
...
void transformer_forward(int token, int pos, Config* p, TransformerWeights* w, RunState* s) {
    ...
    for (int l = 0; l < p->n_layers; l++) {
        ...
        // qkv matmuls for this position - TIME THESE SEPARATELY
        ...
        // final matmul to get the output of the attention - TIME THIS
        matmul_start = gettime();
        matmul(s->xb2, s->xb, &(w->wo[l * dim * dim]), dim, dim);
        matmul_cycles += (gettime() - matmul_start);

        // record attention time (EXCLUDING matmuls, which are counted separately)
...
void generate(Transformer* model, Tokenizer* t, char* prompt, 
    ...
    (void)infer_start_cycles; // silence unused warning
    ...
    while (pos < p->seq_len && generated_tokens < (steps + num_prompt_tokens)) {
        ...
        // Decode token
        // Decode and print token (ONLY print generated tokens, skip prompt)
        
        if (next == 2) {
            // EOS token - stop generation
            break;
        } else if (next < 3) {
            // Other special tokens (0=unk, 1=bos) - skip printing
    ...
    if (hotspot_total == 0) hotspot_total = 1; // avoid div0
    ...
    // Fixed format: now prints correctly as "21.500000" instead of "21.%06d"
    printf(stdout, "TTFT: %llu cycles (%d.%d seconds)\n",
       ttft_cycles, ttft_sec_int, ttft_sec_frac);
    printf(stdout, "TPS: %d.%d tokens/sec\n", tps_int, tps_frac);
    printf(stdout, "End-to-End: %llu cycles (%d.%d seconds)\n",
       e2e_cycles, e2e_sec_int, e2e_sec_frac);

    printf(stdout, "HOTSPOT BREAKDOWN (%% of inference time):\n");
    // Fixed format: now prints "59.3%" instead of "59.%01d%"
    printf(stdout, "matmul(): %d.%d%%\n", matmul_int, matmul_frac);
    printf(stdout, "Activations (expf, sqrtf): %d.%d%%\n", act_int, act_frac);
    printf(stdout, "Attention: %d.%d%%\n", att_int, att_frac);
    printf(stdout, "FFN: %d.%d%%\n", ffn_int, ffn_frac);
    printf(stdout, "Sampling: %d.%d%%\n", samp_int, samp_frac);

    printf(stdout, "NOTES: Categories add to 100%%. matmul time counted separately.\n");
    printf(stdout, "===================\n");
...
int main(int argc, char *argv[]) {
    ...
    g_program_start_cycles = gettime();   // or getcycles(), if your syscall is named that

    // Default parameters
    char* prompt = "Once upon a time";
    float temperature = 0.0f;
    float topp = 1.0f;
    int steps = 50; // default max *output* tokens
    unsigned long long seed = 12345;

    // argument structure:
    //   llm "<prompt>" <steps> <temperature> <topp> <seed> <test_name>
    ...
    printf(stdout, "DEBUG: argc = %d\n", argc);
    printf(stdout, "DEBUG: prompt = %s\n", prompt);
    printf(stdout, "DEBUG: steps = %d\n", steps);
    printf(stdout, "DEBUG: temperature = %d.%d\n", (int)temperature, 0);
    printf(stdout, "DEBUG: topp = %d.%d\n", (int)topp, 0);
    printf(stdout, "DEBUG: seed = %llu\n", seed);
    printf(stdout, "DEBUG: test_name = %s\n", g_test_name);
    ...
    printf(stdout, "Steps (max output tokens): %d, Temperature: %d.%d, Topp: %d.%d Seed: %llu\n",
           steps, (int)temperature, 0, (int)topp, 0, seed);

    // Reset hotspots for this run
    reset_benchmark_counters();
    
    // Run generation (benchmark-aware)
